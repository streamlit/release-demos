...
- Install any necessary packages in your environment (such as with `pip` and `requirements.txt`). You can find these in Streamlit’s data source tutorials or the data source documentation.
    - If something is missing when you run your app, Streamlit will try to detect that and give you a hint about what to install. We also have some ideas to make this even easier in the future!
- Set up credentials and connection information
- Import and initialize the connection in your app

### Installing packages

For example, with the MySQL connection, you'll need to install SQLAlchemy and the mysqlclient
...
- Passed directly as arguments to st.connection
...
Then, initialize the connection in your Streamlit app by passing the connection name and type to st.connection.
